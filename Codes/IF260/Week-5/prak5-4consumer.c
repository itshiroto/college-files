...
int main() {
  ...
  producerMtx = sem_open("PRAK5-4PRODUCER", O_CREAT, 0644);
  ...
  consumerMtx = sem_open("PRAK5-4CONSUMER", O_CREAT, 0644);
  ...
  for (int i = 0; i < 10; i++) {
    if (ptr->flag == 0) {
      sem_wait(consumerMtx);
    }
    printf("Data: %s\n", ptr->data);
    ptr->flag = 0;
    sem_post(producerMtx);
  }
  sem_close(producerMtx);
  sem_close(consumerMtx);
  sem_unlink("PRAK5-4CONSUMER");
  sem_unlink("PRAK5-4PRODUCER");
